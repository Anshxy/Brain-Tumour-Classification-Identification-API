...
def predict_tumor_type(model, image):
    """
    Multi-class classification prediction for tumor type
...
binary_model.load_state_dict(torch.load('binary_classification_model.pth'))
...
tumor_model.load_state_dict(torch.load('brain_tumor_classifier.pth'))
...
@app.route('/')
def home():
    return 'Brain Tumor Classification API By Ansh Rawat and Jun Oh'
...
def predict_api():
    ...
    try:
        ...
        if binary_prediction == 'yes':
            # If binary model predicts tumor, classify tumor type
